...
module.exports = {
  ...
  create: Components.detect(function(context, components, utils) {
    ...
    function isDOMComponents(node) {
      const tagName = sourceCode.getText(node.parent.name);
    ...
    return {
      ...
      JSXAttribute: function(node) {
        ...
        const isDOM = configuration.ignoreDOMComponents && isDOMComponents(node);
        if (isRef || !node.value || !node.value.expression || isDOM) {
          return;
        }
